...
const PhotosUploader = ({ addedPhotos, onChange }) => {
  ...
  async function addPhotoByLink(ev) {
    ev.preventDefault();
    const { data: filename } = await axios.post("/api/upload/upload-by-link", {
      link: photoLink,
    });
    onChange((prev) => {
      if (prev === undefined) {
          ...
          addedPhotos.map((photo) => (
            <div className="h-32 flex relative" key={photo}>
              ...
              <button
                ...
                className="absolute bottom-1 right-1 text-white bg-black rounded-2xl py-2 px-2 bg-opacity-50 cursor-pointer"
              >
                <svg
              ...
              <button
                onClick={(ev) => selectMainPhoto(ev, photo)}
                className="absolute bottom-1 left-1 text-white bg-black rounded-2xl py-2 px-2 bg-opacity-50 cursor-pointer"
              >
                {photo === addedPhotos[0] && (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
